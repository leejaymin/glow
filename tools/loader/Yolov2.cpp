...
#include "include/glow/Base/Tensor.h"
...
static int processAndPrintResultsImpl(Tensor *SMT,
  ...
  for (size_t i = 1; i < SMT->dims().size(); i++) {
    if (SMT->dims()[i] > 1) {
      //jemin: DCHECK_EQ(labelsDim, 0) << "More than one dimension of size > 1?";
      labelsDim = i;
    }
  }
  ...
  auto processImageRange = [&](size_t startIndex, size_t endIndex) {
    ...
    while ((streamInputFilenamesMode &&
      ...
      if (!profilingGraph()) {
        ...
        llvm::outs() <<"---------------Jemin Output:---------------"<<'\n';

        Tensor* myOut = bindings.get(outputPH);
        dumpImpl(myOut, 208*208*32);
        myOut->dump();
